<template>
    <div class="pcoded-content">
        <div class="pcoded-inner-content">
            <!-- Main-body start -->
            <div class="main-body">
                <div class="page-wrapper">
                    <!-- Page-header start -->
                    <div class="page-header">
                        <div class="row align-items-end">
                            <div class="col-lg-8">
                                <div class="page-header-title">
                                    <div class="d-inline">
                                        <h4>crear una nueva caracteristica de servicio</h4>
                                        <span>En esta sección se crearan las caracteristicas de los servicios</span>
                                    </div>
                                </div>
                            </div>
                            <div class="col-lg-4">
                                <div class="page-header-breadcrumb">
                                    <ul class="breadcrumb-title">
                                        <li class="breadcrumb-item">
                                            <router-link :to="{name: 'admin'}"><i class="feather icon-home"></i> </router-link>
                                        </li>
                                        <li class="breadcrumb-item"><router-link :to="{name: 'services'}">Sevicios</router-link>
                                        </li>
                                        <li class="breadcrumb-item"><a href="newservice">Nuevo servicio</a>
                                        </li>
                                    </ul>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div class="page-body">
                        <div class="row">
                            <div class="col-md-8">
                                <div style="border-top-color: #00a65a !important;position: relative;border-radius: 3px; background: #ffffff;border-top: 3px solid #d2d6de; margin-bottom: 20px;width: 100%; box-shadow: 0 1px 1px rgba(0,0,0,0.1);" class="box box-primary">
                                    <div class="box-header" style="color: #444;display: block;  padding: 10px; position: relative;border-bottom: 1px solid #f4f4f4;">
                                        <div class="box-title" style="display: inline-block; font-size: 18px;  margin: 0; line-height: 1;">
                                            Crear una nueva caracteristica para un servicio servicio
                                        </div>
                                    </div>
                                    <div class="box-body" style="border-top-left-radius: 0;border-top-right-radius: 0;border-bottom-right-radius: 3px;border-bottom-left-radius: 3px;padding: 10px;">
                                        <div class="form-group">
                                          <label for="title-service">Titulo de la caracteristica</label>
                                          <input type="text" class="form-control" name="name-service" v-model="servicefeatures_title" aria-describedby="emailHelpId" placeholder="ingresa el titulo de la caract6eristica">
                                        </div>
                                        
                                        <div class="form-group">
                                          <label for="">Contenido de la caracteristica </label>
                                          <ckeditor :editor="editor" rows="15"  v-model="servicefeatures_body"></ckeditor>
                                        </div>
                                    </div>
                                </div>
                            </div>
                            <div class="col-md-4">
                                <div style="border-top-color: #00a65a;position: relative;border-radius: 3px; background: #ffffff;border-top: 3px solid #d2d6de; margin-bottom: 20px;width: 100%; box-shadow: 0 1px 1px rgba(0,0,0,0.1);">
                                    <div style="color: #444;display: block;  padding: 10px; position: relative;border-bottom: 1px solid #f4f4f4;">
                                        <div style="display: inline-block; font-size: 18px;  margin: 0; line-height: 1;">
                                            Adicionales
                                        </div>
                                    </div>
                                    <div style="border-top-left-radius: 0;border-top-right-radius: 0;border-bottom-right-radius: 3px;border-bottom-left-radius: 3px;padding: 10px;">
                                        <div class="form-group">
                                            <img v-if="show_photo" style="width:100%" v-bind:src="servicefeatures_photo" alt="" srcset="">
                                          <label for="image-service">Imagen principal del servicio</label>
                                          <input type="file" class="form-control-file"  name="service-post" v-on:change="onImageChange" placeholder="" aria-describedby="fileHelpId">
                                        </div>
                                        <hr>
                                        <div class="form-group">
                                            <label for="category_service"></label>
                                            <div class="form-group">
                                                <label for="tags-post">servicio</label>
                                                <b-select v-model="service_id" :options="services" class="form-control" style="width:100%" value-field="id" text-field="service_name"></b-select>
                                            </div>
                                        </div>
                                        <hr>

                                    </div>
                                    <div style="padding: 0 5px 10px;">
                                        <button type="button" style="padding:5px;background-color: #3c8dbc; border-color: #367fa9;" @click="saveFeatures" class="btn btn-primary btn-lg btn-block">Guardar caracteristica</button>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
    </div>
</template>

<script>
    import ClassicEditor from '@ckeditor/ckeditor5-build-classic';
    import Axios from 'axios';
   import { BFormSelect, BFormSelectOption } from 'bootstrap-vue'
    export default {
        components: {
            'b-select': BFormSelect,
            'b-select-option': BFormSelectOption
        },
        mounted() {

            this.getServices()
        },
        data(){
            return {
                editor: ClassicEditor,
                servicefeatures_body: null,
                servicefeatures_photo:null,
                show_photo:false,
                services: [],
                service_id:null,
                servicefeatures_title:null
            }
        },
        methods: {

            async getServices() {
                await Axios.get('/api/services',{headers:{'Authorization':this.$session.get('Authorization'), 'Accept':'application/json'}}).then(res =>{
                    
                    this.services = res.data;
                     console.log(this.services);
                }).catch(err =>{
                    console.log(err);
                })
            },
            onImageChange(e){
                let input = e.target;
                if (input.files && input.files[0]) {
                    var reader = new FileReader();
                    let vm = this;
                    reader.onload = e => {
                        this.previewImageUrl = e.target.result;
                        vm.servicefeatures_photo = e.target.result;
                        vm.show_photo = true;
                    }
                    reader.readAsDataURL(input.files[0]);
                }
            },
            async saveFeatures(e) {
                let me =this;
                let url = '/api/service_features' //Ruta que hemos creado para enviar una usuario y guardarlo
                this.$v.$touch()
                if (this.$v.$invalid) {
                    this.submitStatus = 'ERROR'
                } else {
                  
                    await Axios.post(url,{ //estas variables son las que enviaremos para que crear el usuario
                    'servicefeatures_title':this.servicefeatures_title,
                    'servicefeatures_photo':this.servicefeatures_photo,
                    'servicefeatures_body':this.servicefeatures_body,
                    'service_id':this.service_id,
                    },
                    {
                        headers:{'Accept':'application/json','Authorization' :this.$session.get('Authorization')}
                    }).then(function (res) {
                        console.log(res);
                    })
                    .catch(function (error) {
                        console.log(error);
                    });
                }
                e.preventDefault();

            },
        },
        validations: {

        }
    }
</script>
